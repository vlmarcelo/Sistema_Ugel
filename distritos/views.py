...
from .forms import DistritoModelForm
...
class DistritoCreateView(AccessUserRequiredMixin, CreateView):
	form_class    = DistritoModelForm
	models        = Distrito
...
class DistritoUpdateView(AccessUserRequiredMixin, UpdateView):
    form_class      = DistritoModelForm
    models          = Distrito
    success_url     = reverse_lazy('distrito:list')
...
class DistritoDeleteView(AccessUserRequiredMixin, DeleteView):
    models        = Distrito
    queryset      = Distrito.objects.all()
    success_url   = reverse_lazy('distrito:list')
...
class DistritoDetailView(AccessUserRequiredMixin, DetailView):
    model = Distrito 
...
class DistritoListView(AccessUserRequiredMixin, PaginationMixin, ListView):
    template_name = 'distrito_list.html'
    model         = Distrito # Al usar el mixin se usa object
    paginate_by   = 20
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Toma el queryset por defecto
            ...
            return super(DistritoListView, self).get(self, request, *args, **kwargs)
    ...
    def json_to_response(self):
        ...
        return JsonResponse(DistritoListView, safe=False)
    ...
    def get_queryset(self):
        if self.kwargs.get('variable'):
            ...
            queryset = super(DistritoListView, self).get_queryset()
        return queryset
